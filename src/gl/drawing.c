...
#undef GetP
        ...
        if (instancecount == 1 || hardext.esversion == 1) {
            if (!iindices && !sindices)
                ...
                // a natively-bound element buffer): sindices/iindices are CLIENT-side pointers
                // here; with an EBO natively bound the pointer is treated as an offset ->
                // GL_INVALID_OPERATION on every such draw. Force native EBO=0, restore after.
